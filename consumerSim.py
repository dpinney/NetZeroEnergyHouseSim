...
plt.plot_date(dates, [float(x.get('heating_demand[kW]', 0.0)) for x in data['out_house.csv']], '-', label="Heating")
plt.plot_date(dates, [float(x.get('cooling_demand[kW]', 0.0)) for x in data['out_house.csv']], '-', label="Cooling")
plt.plot_date(dates, [float(x.get('power.real[kW]', 0.0)) for x in data['out_waterheater.csv']], '-', label="Water Heating")
plt.plot_date(dates, [float(x.get('power.real[kW]', 0.0)) for x in data['out_responsive.csv']], '-', label="Responsive Loads")
plt.plot_date(dates, [float(x.get('power.real[kW]', 0.0)) for x in data['out_unresponsive.csv']], '-', label="Unresponsive Loads")
plt.plot_date(dates, [float(x.get('power.real[kW]', 0.0)) for x in data['out_ev_charger.csv']], '-', label="EV Charger")
plt.plot_date(dates, [-1.0 * float(x.get('VA_Out.real[kW]', 0.0)) for x in data['out_solar_inv.csv']], '-', label="Solar")
ax = plt.gcf().axes[0]
